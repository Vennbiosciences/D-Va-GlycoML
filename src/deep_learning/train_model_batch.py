...
def train_model_batch(train_path, test_path, saved_model_path):
    """
    ...
    for i in range(200):
        ...
        if i > 1:
            for j in range(len(test_name_lists)):
                sims_difference_lists[j] = np.median(cos_sims_current_lists[j]) - np.median(cos_sims_last_lists[j])
                print(f"The median difference of Cos Similarity for {test_name_lists[j]}: {sims_difference_lists[j]}")
        ...
        # Stop when the median of cos similarity is greater than 0.98
        if np.median(cos_sims_current_lists[0]) > 0.98:
            break
...
def user_interface(train_path, test_path, saved_model_path):
    ...
    :return:
    """
    #  python train_model_batch.py -TA=/data/Training-01-Human-285/N-GP-PKL
    ...
    train_model_batch(train_path, test_path, saved_model_path)
...
if __name__ == "__main__":
    ...
    # -SM=/data/saved_models/train-15_test-4_batch-8_14-Apr-2022
    #
    # python train_model_batch.py -TA=D:\data\Training-01-Human-285\N-GP-PKL
    ...
    # nohup python -u train_model_batch.py -TA=/data/Training-01-Human-285/N-GP-PKL
    # -TE=/data/Testing-01-Different-HCD-energies-24/Energy-01-HCD-15-20-34-37-40/N-GP-PKL
    # -SM=/data/saved_models/train-15_test-4_batch-8_14-Apr-2022
