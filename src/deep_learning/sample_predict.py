...
from spectral_library.calculate_fragment_mz import CalculateFragmentMZ, amino_acid_monosaccharide_zero_codes
import os
import pickle
import csv
from pyteomics import mass
from spectral_library.parse_msp_data import amino_acid_monosaccharide_name_parse
...
#####################################
#df = pd.read_csv("../input_files/synth.peptides.plus.decoys.csv")
df = pd.read_csv("../input_files/synth.peptides.plus.decoys.csv")
...
for i, pred in enumerate(sample_pred):
    ...
    pep_seq = reverse_one_hot_encode(seq, amino_acid_monosaccharide_zero_codes)
...
with open('../output_files/sample_predictions.csv', 'a') as f:
    writer = csv.writer(f)
    for line in lines:
