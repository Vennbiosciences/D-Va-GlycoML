...
Modified on 26 May 2022, in order to do the transfer learning.
########################################################################################################################
"""
__author__ = 'ZLiang'
...
def train_model_transfer(train_path, transfer_model, saved_model_path):
    """
    :param train_path: the pkl files for the training;
    :param transfer_model: the pth file of transfer model for the initial weights;
    ...
    transfer_pth = Path(transfer_model)
    assert transfer_pth.is_file(), "Input transfer learning model is wrong!"
    saved_model_folder = Path(saved_model_path)
    assert saved_model_folder.is_dir(), "Output saved model path is wrong!"

    trainer.load(transfer_pth)
    print(f"The started transfer learning model is: {transfer_pth}")
...
def user_interface(train_path, transfer_model, saved_model_path):
    """
    :param train_path: path for the training
    :param transfer_model: the initial transfer model;
    :param saved_model_path: path for the saved model
    :return:
    """
    # python train_model_cyno.py -TA=/data/Training-01-Human-285/N-GP-PKL
    #   -TM=/data/saved_models/train-345-76-Full-top-1_epoch-50_batch-64_lr-4_24-May-2022/model-49.pth
    #   -SM=/data/saved_models/train-15_test-4_batch-8_14-Apr-2022
    train_model_transfer(train_path, transfer_model, saved_model_path)


"""
Input parameters for the user interface:
    1   Input Training Path    
    2   Input Transfer Model 
...
parser.add_argument('--TrainingPath', '-TA',
                    ...
                    required=False)
parser.add_argument('--TransferModel', '-TM',
                    help='Input Saved Transfer Model parameter，required，no default. Such as '
                         '/data/saved_models/train-345-76-Full-top-1_epoch-50_batch-64_lr-4_24-May-2022/model-49.pth.',
                    required=False)
...
if __name__ == "__main__":
    ...
    #  -TM=D:\data\saved_models\train-345-76-Full-top-1_epoch-50_batch-64_lr-4_24-May-2022\model-49.pth
    ...
    try:
        user_interface(args.TrainingPath, args.TransferModel, args.SavedModelPath)
